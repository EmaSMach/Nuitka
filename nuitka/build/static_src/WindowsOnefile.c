...
static void appendCharSafeW(wchar_t *target, char c, size_t buffer_size) {
    while (*target != 0) {
        target++;
        buffer_size -= 1;
    }
...
static void appendStringSafeW(wchar_t *target, char const *source, size_t buffer_size) {
    while (*target != 0) {
        target++;
        buffer_size -= 1;
    }
...
static char const *readFilename(HANDLE exe_file) {
    ...
    for (;;) {
        DWORD read_size;
        BOOL bool_res = ReadFile(exe_file, w, 1, &read_size, NULL);
...
static long long readSizeValue(HANDLE exe_file) {
    long long result;
    DWORD read_size;
    BOOL bool_res = ReadFile(exe_file, &result, sizeof(long long), &read_size, NULL);
    assert(bool_res);
    assert(read_size == sizeof(long long));
...
int main(int argc, char **argv) {
    ...
    appendCharSafeW(payload_path, '\\', sizeof(payload_path));
    ...
    res = SetFilePointer(exe_file, -sizeof(long long), NULL, FILE_END);
    assert(res != INVALID_SET_FILE_POINTER);

    DWORD read_size;

    long long start_pos = readSizeValue(exe_file);

    // printf("Start at %lld\n", start_pos);

    res = SetFilePointer(exe_file, start_pos, NULL, FILE_BEGIN);
    assert(res != INVALID_SET_FILE_POINTER);
    ...
    for (;;) {
        char const *filename = readFilename(exe_file);
        ...
        appendCharSafeW(target_path, '\\', sizeof(target_path));
        appendStringSafeW(target_path, filename, sizeof(target_path));
        ...
        long long file_size = readSizeValue(exe_file);

        while (file_size > 0) {
            static char chunk[4096];

            long chunk_size = min(file_size, sizeof(chunk));
